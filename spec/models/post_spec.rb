...
RSpec.describe Post, type: :model do
  ...
  let(:respondant) { create(:user) }
  let(:thread) { create(:post, user: author, created_at: 2.days.ago) }
  let(:post1) { create(:post, user: respondant, parent: thread, created_at: 15.minutes.ago)}
  let(:post2) { create(:post, user: author, parent: thread, created_at: 5.minutes.ago)}
  let(:new_thread) { create(:post) }
  ...
  it "is archivable" do
    ...
    thread.reload
    expect(thread.archived_at).to be < 1.minute.ago
  end

  it "can't get reassigned to a new thread" do
    pending "handle in controller"
  end
end
